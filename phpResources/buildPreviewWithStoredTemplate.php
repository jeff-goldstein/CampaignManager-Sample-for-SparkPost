...
$recipientSelect 		= $_POST["recipientSelect"];
$enteredRecipientData	= $_POST["recipientJSON"];
$globalsub				= $_POST["globalsub"];

$rec_sub = " ";  //set to a default where I know nothing was entered
if (strlen($enteredRecipientData))
{
	...
	{
		//get the first manually entered recipient data
		...
		$recipientSelect = "User Entered Data";
	}
}
else
{   //No data was entered, so let's look at the recipient select entry
	if ($recipientSelect != "Recipient List Not Entered")
	{
		$curl = curl_init();
    	$url = $apiroot . "/recipient-lists/" . $recipientSelect . "?show_recipients=true";
    ...
    {	// Looks like they didn't enter any user data or select a recipient list.  We will move forward no recipeint data.
    	// There still might be global data though
    	$rec_sub = "";
    }
}
...
else 
{
    ...
    {
        $error_out = "<h3>Matching Problem between selected Template: <u>" . $template . "</u> and Recipient List: <u>" . $recipientSelect . "</u></h3>";
