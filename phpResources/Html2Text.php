 ...
 */
 ...
 /* Some changes have been made by Jeff Goldstein in Oct 2017
  * Added code for <code>
  * Changed <td> replacement string
  * Created a H1 replacement instead of treating it exactly the same as all the other <Hx> tags
  * Changed width from 70 to 132
  * Removed the function call toupper on H1, H2-6, TH & strong because SparkPost strings may be caught in the 'toupper' call and change good
  *   SparkPost syntax and variables to all uppercase syntax and fieldnames that don't work.
  */
...
class Html2Text
{
    ...
    /**
     * List of pattern replacements corresponding to patterns searched for CODE body.
     *
     * @type array
     * @see $preSearch
     */
    protected $codeReplace = array(
        '<br>&nbsp;&nbsp;&nbsp;&nbsp;',
        '&nbsp;&nbsp;&nbsp;&nbsp;',
        '&nbsp;',
        '',
    ...
    protected $options = array(
        ...
        'width' => 132,          //  Maximum width of the formatted text, in columns.
    ...
    */
    ...
    protected function convertCode(&$text)
    {
        // get the content of Code element
        while (preg_match('/<code[^>]*>(.*)<\/code>/ismU', $text, $matches)) {
            // Replace br tags with newlines to prevent the search-and-replace callback from killing whitespace
            $this->codeContent = preg_replace('/(<br\b[^>]*>)/i', "\n", $matches[1]);

            // Run our defined tags search-and-replace with callback
            $this->codeContent = preg_replace_callback(
                $this->callbackSearch,
                array($this, 'pregCallback'),
                $this->codeContent
            );
            ...
            // convert the content
            $this->codeContent = sprintf(
                '<br>&nbsp;&nbsp;&nbsp;&nbsp;%s<br>',
                preg_replace($this->codeSearch, $this->codeReplace, $this->codeContent)
            );

            // replace the content (use callback because content can contain $0 variable)
            $text = preg_replace_callback(
                '/<code[^>]*>.*<\/code>/ismU',
                array($this, 'pregCodeCallback'),
                $text,
                1
            );
    ...
    protected function pregPreCallback(/** @noinspection PhpUnusedParameterInspection */ $matches)
    {
     ...
     /**
     * Callback function for preg_replace_callback use in CODE content handler.
     *
     * @param  array  $matches PREG matches
     * @return string
     */
    protected function pregCodeCallback(/** @noinspection PhpUnusedParameterInspection */ $matches)
    {
